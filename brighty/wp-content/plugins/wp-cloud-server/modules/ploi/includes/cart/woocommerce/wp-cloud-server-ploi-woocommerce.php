<?php
/**
 * WooCommerce Functions.
*/

// Exit if accessed directly.
if ( ! defined( 'ABSPATH' ) ) exit;

/**
 * Add the field to the checkout
 */
function ploi_custom_checkout_fields( $server, $checkout ) {

	if ( 'Ploi' == $server['module'] ) {

		if ( 'userselected' == $server['region'] ) {

			echo '<div id="website-location"><h3>' . __('Website Location') . '</h3>';
	
			woocommerce_form_field( 'web_hosting_region', array(
				'type'          => 'select',
				'class'         => array('web-hosting-region form-row-wide'),
				'label'         => __('Select the location for your website?'),
				'required'    => true,
				'options'     => wpcs_ploi_regions_array(),
				'default' => 'lon', 
				$checkout->get_value( 'web_hosting_region' ))
			);

			echo '</div>';

		}

		if ( '[Customer Input]' == $server['host_name'] ) {

			echo '<div id="server-host-name"><h3>' . __('Server Host Name') . '</h3>';

			woocommerce_form_field( 'server_hostname', array(
				'type'          => 'text',
				'class'         => array('server-host-name form-row-wide'),
				'label'         => __('Please enter a hostname for your new server?'),
				'required'    	=> true,
				'placeholder'   => __('host-name'),
			), $checkout->get_value( 'server_hostname' ));

			echo '</div>';

		}

		if ( '[Customer Input]' == $server['root_domain'] ) {

			echo '<div id="domain-name-field"><h3>' . __('Domain Name') . '</h3>';

			woocommerce_form_field( 'domain_name', array(
				'type'          => 'text',
				'class'         => array('domain-name form-row-wide'),
				'label'         => __('Please enter a domain name for your new website?'),
				'required'    	=> true,
				'placeholder'   => __('example.com'),
			), $checkout->get_value( 'domain_name' ));

			echo '</div>';

		}
	}
}
add_action( 'wpcs_wc_custom_checkout_fields', 'ploi_custom_checkout_fields',10, 2 );

/**
 * Process the checkout
 */
function ploi_custom_checkout_field_process( $server ) {

	if ( 'Ploi' == $server['module'] ) {
		if ( '[Customer Input]' == $server['host_name'] ) {
			if ( ! $_POST['server_hostname'] ) {
				wc_add_notice( __( 'Please enter a valid hostname for your server.' ), 'error' );
			}
		}

		if ( '[Customer Input]' == $server['root_domain'] ) {
			if ( ! $_POST['domain_name'] ) {
				wc_add_notice( __( 'Please enter a valid domain name for your website.' ), 'error' );
			}
		}
	}
}
add_action('wpcs_wc_custom_checkout_field_process', 'ploi_custom_checkout_field_process');

/**
 * Update the order meta with field value
 */
function ploi_custom_checkout_field_update_order_meta( $order_id ) {

    return;

}
add_action( 'wpcs_wc_custom_checkout_field_update_order_meta', 'ploi_custom_checkout_field_update_order_meta' );