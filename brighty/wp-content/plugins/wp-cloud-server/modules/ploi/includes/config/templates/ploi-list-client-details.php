...
function wpcs_ploi_client_details_template ( $tabs_content, $page_content, $page_id ) {
	
	if ( 'ploi-client-details' !== $tabs_content ) {
		return;
	}
	...
	<div class="uk-overflow-auto">
		...
		<table class="uk-table uk-table-striped">
			<thead>
				<tr>
					<th class="col-title"><?php esc_html_e( 'User ID', 'wp-cloud-server' ); ?></th>
			...
			<tbody>
				<?php
				if ( !empty( $clients_data['Ploi'] ) ) {
					foreach ( $clients_data['Ploi'] as $key => $client_data ) {
						?>
						<tr>
							...
							<td><?php echo ( isset( $client_data['domain_name'] ) ) ? $client_data['domain_name'] : ''; ?></td>
							<td><?php echo ( isset( $client_data['host_name'] ) ) ? $client_data['host_name'] : ''; ?></td>
	...
	<?php
	if ( isset( $clients_data['Ploi'] ) ) {
		foreach ( $clients_data['Ploi'] as $key => $client ) {
			?>
			<div id="client-details-modal-<?php echo $client['host_name']; ?>" uk-modal>
				<div class="server-modal uk-modal-dialog uk-modal-body">
					...
					<div class="server-info uk-modal-body" uk-overflow-auto>
						<table class="server-info uk-table uk-table-striped">
							<tbody>
								...
								<tr>
									<td><?php esc_html_e( 'Host Name', 'wp-cloud-server' ); ?></td>
					...
					<hr>
					<div>
						<a class="uk-button uk-button-danger uk-align-left uk-margin-remove-bottom" href="#delete-ploi-client-<?php echo $client['host_name']; ?>" uk-toggle><?php esc_attr_e( 'DELETE', 'wp-cloud-server' ) ?></a>
					</div>
				</div>
				<div id="delete-ploi-client-<?php echo $client['host_name']; ?>" uk-modal>
					<div class="server-modal uk-modal-dialog uk-modal-body">
						<div class="uk-modal-body">
							<p class="uk-text-lead">
								<?php 
								$client_name = "{$client['first_name']} {$client['last_name']}";
								esc_html_e( "Take care! This will delete the website from {$client_name}'s client information! It cannot be reversed!", 'wp-cloud-server' ); ?></p>
							<p><?php esc_html_e( "Please confirm by clicking the 'CONFIRM' button below!", 'wp-cloud-server' ); ?></p>
						</div>
						<div class="uk-modal-footer uk-text-right">
							<form method="post" action="<?php echo admin_url( 'admin-post.php' ) ?>">
								<input type="hidden" name="action" value="handle_ploi_client_action">
								<input type="hidden" name="wpcs_ploi_client_action" value="delete">
								<input type="hidden" name="wpcs_ploi_client_host_name" value="<?php echo $client['host_name'];?>">
								<input type="hidden" name="wpcs_ploi_client_user_id" value="<?php echo $client['user_id'];?>">
								<?php wp_nonce_field( "handle_ploi_client_action", "wpcs_handle_ploi_client_action_nonce_{$client['host_name']}" ); ?>
								<div class="uk-button-group uk-margin-remove-bottom">
									...
									<?php wpcs_submit_button( 'Confirm Delete', 'danger', "delete_server_{$client['host_name']}", false ); ?>
								</div>
							</form>
						</div>
				</div>
...
add_action( 'wpcs_control_panel_tab_content', 'wpcs_ploi_client_details_template', 10, 3 );