...
/**
 * WP Cloud Server - Vultr Module API Class
 *
...
class WP_Cloud_Server_Vultr_API {
	 ...
	 *  @var string
	 */
	private static $api_base_url = 'https://api.vultr.com/v1';
	...
	public function __construct() {
		...
		self::$api_token = get_option( 'wpcs_vultr_api_token' );
	 ...
	 */
	public function wpcs_vultr_check_api_health() {

		// Check if the health is temporarily cached.
		$health = get_transient( 'wpcs_vultr_api_health' );
		 ...
		 */
		$api_response = wpcs_vultr_call_api_health_check();
			
		update_option( 'wpcs_vultr_api_last_health_response', $api_response );

		if ( wpcs_vultr_api_response_not_valid( $api_response ) ) {

			// Can't connect. Not healthy.
			delete_transient( 'wpcs_vultr_api_health' );
			update_option( 'wpcs_dismissed_vultr_api_notice', FALSE );

			// Update the log event once for current failure
			if ( !get_option( 'wpcs_vultr_api_health_check_failed', false ) ) {
				wpcs_log_event( 'Vultr', 'Failed', 'The Vultr API Health Check Failed!' );
				update_option( 'wpcs_vultr_api_health_check_failed', true );
			}
			return false;
		}
			
		// Update Vultr Module Data
		WP_Cloud_Server_Vultr_Settings::wpcs_vultr_update_servers();

		// API connection is healthy. Cache result for three minutes
		wpcs_vultr_set_transient( 'wpcs_vultr_api_health', 'ok', 60 );
		
		if ( get_option( 'wpcs_vultr_api_health_check_failed' ) ) {
			wpcs_vultr_log_event( 'Vultr', 'Success', 'The Vultr API Health Check Completed Successfully' );

			// Clear the latched failed indication now that API is ok
			update_option( 'wpcs_vultr_api_health_check_failed', false );
		}
	...
	public function call_api( $model, $api_data = null, $cache = false, $cache_lifetime = 900, $request = 'GET', $enable_response = false, $function = null ) {
		...
		if ( ( !isset( $module_list[ 'Vultr' ]['status'] ) ) || ( 'active' !== $module_list[ 'Vultr' ]['status'] ) ) {
			return false;
		}
		...
		if ( 'POST' === $request ) {
			...
			$response = self::remote_post( trailingslashit( self::$api_base_url ) . $model , $args );
		} else if ( 'PUT' === $request ) {
			...
			$args['body'] = $api_data;
			$args['method'] = 'GET';
			$response = self::remote_get( trailingslashit( self::$api_base_url ) . $model , $args );
		...
		$response_code = (int) $response['response']['http_code'];
			
		// if ( ( $response_code <= 200 && $response_code > 300 ) || $response_code > 300 ) {
		if ( ( $response_code < 200 ) || $response_code > 300 ) {
			// Save the last API error response for debugging
			update_option( 'wpcs_vultr_api_last_response', $body );
			//wpcs_vultr_api_response_data( $model, $body, $data, $api_data, $function );
			return false;
		}
			
		// Save the last API response for debugging
		update_option( 'wpcs_vultr_api_last_response', $response );
		//wpcs_vultr_api_response_data( $model, $body, $data, $api_data, $function );
	...
	private function get_do_api_response_cache( $model ) {
		return get_transient( 'wpcs_vultr_api_response_' . md5( $model ) );
	}
	...
	private function set_do_api_response_cache( $model = null, $data = null, $lifetime = 900 ) {
		...
		wpcs_vultr_set_transient( 'wpcs_vultr_api_response_' . md5( $model ), $data, $lifetime );
	}
