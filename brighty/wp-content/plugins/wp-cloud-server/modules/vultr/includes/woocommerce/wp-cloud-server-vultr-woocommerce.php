 ...
 */
function vultr_custom_checkout_fields( $server, $checkout ) {

	if ( 'Vultr' == $server['module'] ) {
		...
		if ( 'userselected' == $server['region'] ) {
			...
			woocommerce_form_field( 'web_hosting_region', array(
				...
				'required'    => true,
				'options'     => wpcs_vultr_regions_array(),
				'default' => 'lon', 
...
add_action( 'wpcs_wc_custom_checkout_fields', 'vultr_custom_checkout_fields',10, 2 );

/**
 * Process the checkout
 */
function vultr_custom_checkout_field_process( $server ) {
...
add_action('wpcs_wc_custom_checkout_field_process', 'vultr_custom_checkout_field_process');

/**
 * Update the order meta with field value
 */
function vultr_custom_checkout_field_update_order_meta( $order_id ) {

    return;

}
add_action( 'wpcs_wc_custom_checkout_field_update_order_meta', 'vultr_custom_checkout_field_update_order_meta' );