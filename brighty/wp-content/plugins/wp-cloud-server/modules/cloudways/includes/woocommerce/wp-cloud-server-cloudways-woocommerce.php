 ...
 */
function cloudways_custom_checkout_fields( $server, $checkout ) {

	if ( 'Cloudways' == $server['module'] ) {
		...
		if ( 'userselected' == $server['region'] ) {
			...
			woocommerce_form_field( 'web_hosting_region', array(
				...
				'required'    => true,
				'options'     => wpcs_cloudways_regions_array(),
				'default' => 'lon', 
...
add_action( 'wpcs_wc_custom_checkout_fields', 'cloudways_custom_checkout_fields',10, 2 );

/**
 * Process the checkout
 */
function cloudways_custom_checkout_field_process( $server ) {
...
add_action('wpcs_wc_custom_checkout_field_process', 'cloudways_custom_checkout_field_process');

/**
 * Update the order meta with field value
 */
function cloudways_custom_checkout_field_update_order_meta( $order_id ) {

    return;

}
add_action( 'wpcs_wc_custom_checkout_field_update_order_meta', 'cloudways_custom_checkout_field_update_order_meta' );