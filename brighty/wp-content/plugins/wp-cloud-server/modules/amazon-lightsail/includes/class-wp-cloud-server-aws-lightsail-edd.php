...
/**
 * WP Cloud Server - AWS Lightsail Module EDD Cart Class
 *
...
class WP_Cloud_Server_AWS_Lightsail_Cart_EDD {
	...
	public function __construct() {
		...
		add_action( 'wpcs_edd_purchase_complete_create_service', array( $this, 'wpcs_aws_lightsail_purchase_complete_create_service' ), 10, 2 );
		add_action( 'wpcs_wc_purchase_complete_create_service', array( $this, 'wpcs_aws_lightsail_purchase_complete_create_service' ), 10, 2 );

		// Add Custom Checkout Fields for the EDD checkout
		add_action( 'edd_purchase_form_user_info_fields', array( $this, 'wpcs_aws_lightsail_purchase_form_custom_fields' ) );
		add_action( 'edd_checkout_error_checks', array( $this, 'wpcs_aws_lightsail_purchase_form_error_checks' ), 10, 2 );
		//add_action( 'edd_payment_personal_details_list', array( $this, 'wpcs_aws_lightsail_purchase_client_details_list' ) );
		
		// Hook into Easy Digital Downloads filters.
		add_filter( 'edd_purchase_form_required_fields', array( $this, 'wpcs_aws_lightsail_purchase_required_checkout_fields' ) );
   ...
   /**
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_aws_lightsail_purchase_form_custom_fields() {
		...
		if ( ( 'AWS Lightsail' == $plan_module ) && ( ( '[Customer Input]' == $droplet_data['host_name'] ) || ( 'userselected' == $droplet_data['region'] ) ) ) {
			
            if ( '[Customer Input]' == $droplet_data['host_name'] ) {
				?>
			     <legend class="website-details"><?php esc_html_e( 'New Server Details', 'wp-cloud-server-aws-lightsail' ); ?></legend>
			
                    <p>
						<label class="edd-label" for="edd_host_name"><?php esc_html_e( 'Host Name', 'wp-cloud-server-aws-lightsail' ); ?><span class="edd-required-indicator">*</span></label>
            			<input class="edd-input required" type="text" name="edd_host_name" id="edd_host_name" placeholder="<?php esc_attr_e( 'host-name', '-aws-lightsail' ); ?>" value=""/>
				    </p>
 ...
 -->
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_aws_lightsail_purchase_required_checkout_fields( $required_fields ) {
		
		// Retrieve the module for the plan in the checkout cart
		$plan_module = WP_Cloud_Server_Cart_EDD::wpcs_edd_get_plan_module();
		
		if (  'AWS Lightsail' == $plan_module ) {
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_aws_lightsail_purchase_form_error_checks( $valid_data, $data ) {
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_aws_lightsail_purchase_client_details_list( $payment_meta, $user_info ) {
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_aws_lightsail_purchase_complete_create_service( $module_name, $data ) {
		...
		if (  'AWS Lightsail' == $module_name ) {
			...
			$ssh_key_id	= call_user_func("wpcs_aws_lightsail_ssh_key", $server_ssh_key_name, $server_region );
					
			// Set-up the data for the new AWS Lightsail Server
    		$app_data = array(
       	 		"instanceNames"		=>	array( $host_name_only ),
		        "bundleId"			=>	$server_plan_id,
 			    "blueprintId"		=> 	$server_os_id,
    		);
				...
				if ( !empty( $startup_scripts ) ) {
					foreach ( $startup_scripts as $key => $script ) {
						if ( $user_meta == $script['name'] ) {
							...
							$app_data["userData"] = $startup_script;
						}	
					}
				}
			}

			if ( isset( $ssh_key_id ) && $ssh_key_id ) {
			...
			$response		= wpcs_aws_lightsail_call_api_create_server( $app_data, false, $server_region );
			
			$info['response'] = $response;
			
			update_option( 'wpcs_aws_server_info', $info );
