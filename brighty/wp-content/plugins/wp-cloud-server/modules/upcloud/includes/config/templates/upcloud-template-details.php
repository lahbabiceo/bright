...
function wpcs_upcloud_template_details_template( $tabs_content, $page_content, $page_id ) {
	
	if ( 'upcloud-template-details' !== $tabs_content ) {
		return;
	}
	...
	<div class="uk-overflow-auto">
		<h2 class="uk-margin-remove-top uk-heading-divider"><?php _e( 'UpCloud Templates', 'wp-cloud-server' ); ?></h2>
	<table class="uk-table uk-table-striped">
    	<thead>
        	<tr>
					...
					<th><?php _e( 'Host Name', 'wp-cloud-server' ); ?></th>
    	...
    	<tbody>
			<?php
				...
				$templates = $module_data['UpCloud']['templates'];
		...
		<?php
			        ...
			        <div id="managed-template-modal-<?php echo $server_label; ?>" uk-modal>
    			        <div class="template-modal uk-modal-dialog uk-modal-body">
							...
							<form method="post" action="<?php echo admin_url( 'admin-post.php' ) ?>">
								<input type="hidden" name="action" value="handle_edit_upcloud_template">
						        	<div class="content">
										<table class="form-table" role="presentation">
											<tbody>
												<tr>
													...
													<td>
														<?php wpcs_upcloud_template_name( $template['name'] ); ?>
													</td>
												</tr>
												...
												<tr>
													...
													<td>		
														<?php wpcs_upcloud_template_hostname( $template['host_name_label'] ); ?>
													</td>
												</tr>
												<tr>
													<th scope="row">Server Image:</th>
													<td>		
														<?php wpcs_upcloud_template_type( $template['image_name'] ); ?>
													</td>
												</tr>
												<tr>
													<th scope="row">Server Region:</th>
													<td>
														<?php wpcs_upcloud_template_regions( $template['region_name'] ); ?>
													</td>
												</tr>
												<tr>
													<th scope="row">Server Size:</th>
													<td>
														<?php wpcs_upcloud_template_size( $template['size_name'] ); ?>
													</td>
												</tr>
												<tr>
													<th scope="row">Admin SSH Key:</th>
													<td>
														<?php wpcs_upcloud_template_ssh_key( $template['ssh_key_name'] ); ?>
													</td>
												</tr>
												<tr>
													<th scope="row">Startup Script:</th>
													<td>
														<?php wpcs_upcloud_template_startup_script( $template['user_data'] ); ?>
													</td>
												</tr>
												<tr>
													<th scope="row">Enable Server Backups:</th>
													<td>
														<?php wpcs_upcloud_template_enable_backups( $template['backups'] ); ?>
													</td>
												</tr>
												<tr>
													<th scope="row">Sites using template:</th>
													<td>
														<?php wpcs_upcloud_template_site_counter( $template['site_counter'] ); ?>
													</td>
												</tr>
											</tbody>
										</table>
					        ...
					        <a class="uk-button uk-button-danger uk-align-left uk-margin-remove-bottom" href="#delete-template-<?php echo $server_label; ?>" uk-toggle><?php esc_attr_e( 'DELETE', 'wp-cloud-server' ) ?></a>
								...
								<?php wpcs_submit_button( 'Update', 'secondary', 'update_upcloud_template', false ); ?>
								</p>
				...
				<div class="uk-modal-body">
					...
					<div class="uk-modal-footer uk-text-right">
						<form method="post" action="<?php echo admin_url( 'admin-post.php' ) ?>">
							<input type="hidden" name="action" value="handle_delete_upcloud_template">
							<input type="hidden" name="wpcs_upcloud_confirm_template_delete" value="true">
							<input type="hidden" name="wpcs_upcloud_confirm_template_id" value="<?php echo $template['name'];?>">
							<?php
							wp_nonce_field( 'wpcs_handle_delete_upcloud_template', 'wpcs_handle_delete_upcloud_template' );
							?>
...
add_action( 'wpcs_control_panel_tab_content', 'wpcs_upcloud_template_details_template', 10, 3 );