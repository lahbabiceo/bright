...
class Zorem_Woo_Customer_Email_Verification {
	...
	*/
	public function admin_notice_pro_update() { 		
		...
		if ( get_option('wc_cev_pro_notice_ignore') ) {
			return;
		}
		...
		<div class="notice updated notice-success cev-dismissable-notice">
			...
			<p>The pro version allows you to require customers to verify their email address before they can checkout on your store, you can delay the new account email notification to after successful verification, require verification when the email address is updated from the account, set validation code expiration and more..</p>
			<a class="button-primary btn_pro_notice" target="blank" href="https://woocommerce.com/products/customer-email-verification/">Upgrade to Pro Now ></a>
		</div>
		<?php
		}
	}	
	...
	*/
	public function cev_pro_notice_ignore() {
		if (isset($_GET['wc-cev-pro-ignore-notice'])) {
			update_option( 'wc_cev_pro_notice_ignore', 'true' );
		}
	}
