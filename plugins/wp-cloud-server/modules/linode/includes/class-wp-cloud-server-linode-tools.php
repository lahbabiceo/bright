...
/**
 * WP Cloud Server - Linode Module Tools Class
 *
 ...
 */
function wpcs_linode_check_api_settings_existance() {
	$api_token = get_option( 'wpcs_linode_api_token' );
	if ( empty( $api_token )  ) {
		return false;
	}
	return true;
 ...
 */
function wpcs_linode_set_transient( $key = null, $value = null, $expiration = 900 ) {
	$transient_keys = get_option( 'wpcs_linode_transient_keys' );
	if ( set_transient( $key, $value, $expiration ) ) {
		$transient_keys[ $key ] = true;
		update_option( 'wpcs_linode_transient_keys', $transient_keys );
		return true;
	}
	return false;
}
 ...
 *  @param string  $key  key/name for transient
 */
function wpcs_linode_delete_transient( $key = null ) {
	$transient_keys = get_option( 'wpcs_linode_transient_keys' );
	$delete = delete_transient( $key );
	if ( ! $delete ) {
		delete_transient( 'wpcs_linode_api_response_' . md5( $key ) );
	}
	if ( $delete ) {
 ...
 *  @since  1.0.0
 */
function wpcs_linode_purge_cache() {
	$transient_keys = get_option( 'wpcs_linode_transient_keys', array() );
 ...
 *  @since  1.0.0
 */
function wpcs_linode_reset_api_settings() {
	if ( current_user_can( 'manage_options' ) ) {
		delete_option( 'wpcs_linode_api_token' );
		wpcs_linode_purge_cache();
	}
}
 ...
 */
function wpcs_linode_api_response_data( $model, $body, $data, $app_data, $function = null ) {
		
	$response_array = get_option( 'wpcs_linode_api_last_response' );
	...
	update_option( 'wpcs_linode_api_last_response', $response_array );
 ...
 *  @since  1.0.0
 */
function wpcs_linode_log_event( $new_event, $new_status, $new_desc ) {
	...
	$log_count = 0;
	$logged_data = get_option( 'wpcs_linode_logged_data', array() );
	...
	update_option( 'wpcs_linode_logged_data', $logged_data );
	
	// Executes after Linode log event has been performed
	do_action( 'wpcs_linode_after_log_event', $logged_data );
		
}

function wpcs_linode_plugin_activation_link($plugin) {
