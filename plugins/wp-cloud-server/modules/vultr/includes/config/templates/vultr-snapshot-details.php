...
function wpcs_vultr_snapshot_details_template ( $tabs_content, $page_content, $page_id ) {
	
	if ( 'vultr-snapshot-details' !== $tabs_content ) {
		return;
	}
	
	$volumes		= wpcs_vultr_call_api_list_snapshots();
	$module_data	= get_option( 'wpcs_module_list' );			
	?>
	<div class="uk-overflow-auto">
		<h3 class="uk-margin-remove-top uk-heading-divider"><?php _e( 'Vultr Snapshots', 'wp-cloud-server-vultr' ); ?></h3>
			<div class="uk-overflow-auto">
    				...
    				<tbody>
						...
						if (!empty( $volumes ) ) {
							foreach ( $volumes as $key => $volume ) {
								?>
        						<tr>
            						<td><?php echo $volume['SNAPSHOTID']; ?></td>
...
add_action( 'wpcs_control_panel_tab_content', 'wpcs_vultr_snapshot_details_template', 10, 3 );
