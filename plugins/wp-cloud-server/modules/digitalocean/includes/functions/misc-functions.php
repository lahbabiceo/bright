<?php
/**
 * The Server Tools functionality for the DigitalOcean Module.
 *
 ...
 */
function wpcs_digitalocean_check_api_settings_existance() {
 ...
 */
function wpcs_digitalocean_set_transient( $key = null, $value = null, $expiration = 900 ) {
	$transient_keys = get_option( 'wpcs_digitalocean_transient_keys' );
	if ( set_transient( $key, $value, $expiration ) ) {
		$transient_keys[ $key ] = true;
		update_option( 'wpcs_digitalocean_transient_keys', $transient_keys );
		return true;
	}
	return false;
}
 ...
 *  @param string  $key  key/name for transient
 */
function wpcs_digitalocean_delete_transient( $key = null ) {
	$transient_keys = get_option( 'wpcs_digitalocean_transient_keys' );
	$delete = delete_transient( $key );
	if ( ! $delete ) {
		delete_transient( 'wpcs_digitalocean_api_response_' . md5( $key ) );
	}
	if ( $delete ) {
 ...
 *  @since  1.0.0
 */
function wpcs_digitalocean_purge_cache() {
	$transient_keys = get_option( 'wpcs_digitalocean_transient_keys', array() );
 ...
 *  @since  1.0.0
 */
function wpcs_digitalocean_reset_api_settings() {
	if ( current_user_can( 'manage_options' ) ) {
		delete_option( 'wpcs_digitalocean_api_token' );
		wpcs_digitalocean_purge_cache();
	}
}
 ...
 */
function wpcs_digitalocean_api_response_data( $model, $body, $data, $app_data, $function = null ) {
		
	$response_array = get_option( 'wpcs_digitalocean_api_last_response' );
	...
	update_option( 'wpcs_digitalocean_api_last_response', $response_array );
 ...
 *  @since  1.0.0
 */
function wpcs_digitalocean_log_event( $new_event, $new_status, $new_desc ) {
	...
	$log_count = 0;
	$logged_data = get_option( 'wpcs_digitalocean_logged_data', array() );
	...
	update_option( 'wpcs_digitalocean_logged_data', $logged_data );
...
function wpcs_plugin_activation_link($plugin) {
...
function wpcs_digitalocean_module_api_connected() {
	return WPCS_DigitalOcean()->settings->wpcs_digitalocean_module_api_connected();
}

function wpcs_digitalocean_update_module_status( $module_name, $new_status ) {
	return WPCS_DigitalOcean()->settings->wpcs_digitalocean_update_module_status( $module_name, $new_status );
}