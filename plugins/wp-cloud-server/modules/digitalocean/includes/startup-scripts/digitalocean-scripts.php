...
/**
 * Digital User-Meta Script for DigitalOcean Plugin
 *
...
/**
 * Returns the Password Install Script
 *
 * @since    1.0.0
 */
function password_install_script( $admin_password ) {

	$hash_password	= str_replace( "/", "\/", crypt( $admin_password, "\$6\$NWg.ndD.jNZrovZh\$") );
  return $script	= <<<EOF
#cloud-config
package_update: true
runcmd:
  - sed -i 's/^root:.*$/root:$hash_password:18400:0:14600:14:::/' /etc/shadow
EOF;
}
	
/**
 * Returns the WordPress Install Script
 *
 * @since    1.0.0
 */
function wordpress_install_script( $domain, $admin_user, $admin_password, $admin_email, $wp_database, $wp_db_user ) {
