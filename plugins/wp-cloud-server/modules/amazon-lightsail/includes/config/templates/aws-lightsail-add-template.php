...
function wpcs_aws_lightsail_add_template_template (  $tabs_content, $page_content, $page_id  ) {
	
	if ( 'aws-lightsail-add-template' !== $tabs_content ) {
		return;
	}

	$server_module		= 'AWS Lightsail';
	...
	//if ( wp_verify_nonce( $nonce, 'do_add_template_nonce' ) && wpcs_check_cloud_provider() ) {
		...
		<div class="content">
			<form method="post" action="options.php">
				<?php
				settings_fields( 'wpcs_aws_lightsail_create_template' );
				wpcs_do_settings_sections( 'wpcs_aws_lightsail_create_template' );
		...
		$debug_data = array(
			"name"		=>	get_option( 'wpcs_aws_lightsail_template_name' ),
			"host_name"	=>	get_option( 'wpcs_aws_lightsail_template_host_name' ),
			"region"	=>	get_option( 'wpcs_aws_lightsail_template_region' ),
			"size"		=>	get_option( 'wpcs_aws_lightsail_template_size' ),
			"image"		=> 	get_option( 'wpcs_aws_lightsail_template_type' ),
			"ssh_key"	=> 	get_option( 'wpcs_aws_lightsail_template_ssh_key' ),
			"user_data"	=>  get_option( 'wpcs_aws_lightsail_template_startup_script_name' ),
			"backups"	=>	get_option( 'wpcs_aws_lightsail_template_enable_backups' ),
		);
	
		if ( get_option( 'wpcs_aws_lightsail_template_name' ) ) {
	
			$server_type					= get_option( 'wpcs_aws_lightsail_template_type' );
			$server_name					= get_option( 'wpcs_aws_lightsail_template_name' );
			$server_host_name				= get_option( 'wpcs_aws_lightsail_template_host_name' );
			$server_region					= get_option( 'wpcs_aws_lightsail_template_region' );
			$server_size					= get_option( 'wpcs_aws_lightsail_template_size' );
			$server_ssh_key					= get_option( 'wpcs_aws_lightsail_template_ssh_key' );
			$server_startup_script			= get_option( 'wpcs_aws_lightsail_template_startup_script_name' );
			$server_backups	        		= get_option( 'wpcs_aws_lightsail_template_enable_backups' );
		...
		delete_option( 'wpcs_aws_lightsail_template_type');
		delete_option( 'wpcs_aws_lightsail_template_name');
		delete_option( 'wpcs_aws_lightsail_template_host_name');
		delete_option( 'wpcs_aws_lightsail_template_region' );
		delete_option( 'wpcs_aws_lightsail_template_size' );
		delete_option( 'wpcs_aws_lightsail_template_ssh_key' );
		delete_option( 'wpcs_aws_lightsail_template_startup_script_name' );
		delete_option( 'wpcs_aws_lightsail_template_enable_backups' );
}
add_action( 'wpcs_control_panel_tab_content', 'wpcs_aws_lightsail_add_template_template', 10, 3 );