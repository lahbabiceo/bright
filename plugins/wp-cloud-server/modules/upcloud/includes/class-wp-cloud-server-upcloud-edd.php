...
/**
 * WP Cloud Server - UpCloud Module EDD Cart Class
 *
...
class WP_Cloud_Server_UpCloud_Cart_EDD {
	...
	public function __construct() {
		...
		add_action( 'wpcs_edd_purchase_complete_create_service', array( $this, 'wpcs_upcloud_purchase_complete_create_service' ), 10, 2 );
		add_action( 'wpcs_wc_purchase_complete_create_service', array( $this, 'wpcs_upcloud_purchase_complete_create_service' ), 10, 2 );

		// Add Custom Checkout Fields for the EDD checkout
		add_action( 'edd_purchase_form_user_info_fields', array( $this, 'wpcs_upcloud_purchase_form_custom_fields' ) );
		add_action( 'edd_checkout_error_checks', array( $this, 'wpcs_upcloud_purchase_form_error_checks' ), 10, 2 );
		//add_action( 'edd_payment_personal_details_list', array( $this, 'wpcs_upcloud_purchase_client_details_list' ), 10, 2 );
		
		// Hook into Easy Digital Downloads filters.
		add_filter( 'edd_purchase_form_required_fields', array( $this, 'wpcs_upcloud_purchase_required_checkout_fields' ) );
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_upcloud_purchase_form_custom_fields() {
		...
		foreach ( $module_data[ $server_name['custom_field1'][0]]['templates'] as $server ) {
			if ( $server_name['custom_field2'][0] == $server['name'] ) {
				...
				if ( ( 'UpCloud' == $plan_module ) && ( ( 'host_name_field' == $droplet_data['host_name'] ) || ( 'userselected' == $droplet_data['region'] ) ) ) {
			
					if ( 'host_name_field' == $droplet_data['host_name'] ) {
						?>
						<legend class="website-details"><?php esc_html_e( 'New Server Details', 'wp-cloud-server-upcloud' ); ?></legend>
			
        				<p>
						<label class="edd-label" for="edd_host_name"><?php esc_html_e( 'Host Name', 'wp-cloud-server-upcloud' ); ?><span class="edd-required-indicator">*</span></label>
            			<input class="edd-input required" type="text" name="edd_host_name" id="edd_host_name" placeholder="<?php esc_attr_e( 'host-name', 'wp-cloud-server-upcloud' ); ?>" value=""/>
						</p>
					...
					if ( 'userselected' == $droplet_data['region'] ) {
						...
						<p>
						<label class="edd-label" for="edd_server_location"><?php esc_html_e( 'Website Location', 'wp-cloud-server-upcloud' ); ?><span class="edd-required-indicator">*</span></label>
			...
			<!--
			<p>
				<label class="edd-label" for="edd_user_name"><?php esc_html_e( 'Username', 'wp-cloud-server-upcloud' ); ?><span class="edd-required-indicator">*</span></label>
            	<input class="edd-input required" type="text" name="edd_user_name" id="edd_user_name" placeholder="<?php esc_attr_e( 'Username', 'wp-cloud-server-upcloud' ); ?>" value=""/>
			</p>
			<p>
				<label class="edd-label" for="edd_user_password"><?php esc_html_e( 'Password', 'wp-cloud-server-upcloud' ); ?><span class="edd-required-indicator">*</span></label>
            	<input class="edd-input required" type="password" name="edd_user_password" id="edd_user_password" placeholder="<?php esc_attr_e( 'Password', 'wp-cloud-server-upcloud' ); ?>" value=""/>
			</p>
			<p>
				<label class="edd-label" for="edd_user_confirm_password"><?php esc_html_e( 'Confirm Password', 'wp-cloud-server-upcloud' ); ?><span class="edd-required-indicator">*</span></label>
            	<input class="edd-input required" type="password" name="edd_user_confirm_password" id="edd_user_confirm_password" placeholder="<?php esc_attr_e('Confirm Password', 'wp-cloud-server-upcloud'); ?>" value=""/>
			</p>
-->
        	<?php
			}					
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_upcloud_purchase_required_checkout_fields( $required_fields ) {
		
		// Retrieve the module for the plan in the checkout cart
		$plan_module = WP_Cloud_Server_Cart_EDD::wpcs_edd_get_plan_module();
		
		if ( 'UpCloud' == $plan_module ) {
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_upcloud_purchase_form_error_checks( $valid_data, $data ) {
		...
		if ( 'UpCloud' == $plan_module ) {
            ...
            //	edd_set_error( 'invalid_host_name', __( 'You must provide a valid host name.', 'wp-cloud-server-upcloud' ) );
        	//}
			
        	//if ( ! isset( $data['edd_domain_name'] ) || $data['edd_domain_name'] == '' ) {
            	// check for a valid domain name
            // 	edd_set_error( 'invalid_domain_name', __( 'You must provide a valid domain name.', 'wp-cloud-server-upcloud' ) );
        	//}

        	//if ( ! isset( $data['edd_user_name'] ) || $data['edd_user_name'] == '' ) {
            	// check for a valid user name
            //	edd_set_error( 'invalid_user_name', __( 'You must provide a valid user name.', 'wp-cloud-server-upcloud' ) );
        	//}
			
        	//if ( ! isset( $data['edd_user_password'] ) || $data['edd_user_password'] == '' ) {
            	// check for a valid password
            //	edd_set_error( 'invalid_password', __( 'You must provide a valid password.', 'wp-cloud-server-upcloud' ) );
        	//}
			
			//if ( ! isset( $data['edd_user_confirm_password'] ) || $data['edd_user_confirm_password'] == '' ) {
            	// check for a valid password confirmation
            //	edd_set_error( 'invalid_confirm_password', __( 'You must provide a valid password.', 'wp-cloud-server-upcloud' ) );
        	//}
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_upcloud_purchase_client_details_list( $payment_meta, $user_info ) {
        ...
        ?>
        <li><?php echo __( 'Host Name:', 'wp-cloud-server-upcloud' ) . ' ' . $host_name; ?></li>
        <li><?php echo __( 'Domain Name:', 'wp-cloud-server-upcloud' ) . ' ' . $domain_name; ?></li>
		<li><?php echo __( 'Username:', 'wp-cloud-server-upcloud' ) . ' ' . $user_name; ?></li>
		<li><?php echo __( 'User Password:', 'wp-cloud-server-upcloud' ) . ' ' . $user_password; ?></li>
		<?php
	 ...
	 *  @since 1.0.0
	 */
	public function wpcs_upcloud_purchase_complete_create_service( $module_name, $data ) {
		...
		if ( 'UpCloud' == $module_name ) {
				...
				if ( !empty( $startup_scripts ) ) {
					foreach ( $startup_scripts as $key => $script ) {
						if ( $user_meta == $script['name'] ) {
							...
							$app_data['user_data']	= $startup_script;
						}	
					}				
				}
			}
			...
			$server_sub_id	= ( isset( $response['uuid'] ) ) ? $response['uuid'] : false ;
