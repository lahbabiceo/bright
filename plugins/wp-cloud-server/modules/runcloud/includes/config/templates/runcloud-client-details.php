...
function wpcs_runcloud_client_details_template ( $tabs_content, $page_content, $page_id ) {
	
	if ( 'runcloud-client-details' !== $tabs_content ) {
		return;
	}
	...
	<div class="uk-overflow-auto">
		...
		<table class="uk-table uk-table-striped">
			...
			<tbody>
				<?php
				if ( !empty( $clients_data['RunCloud'] ) ) {
					foreach ( $clients_data['RunCloud'] as $key => $client_data ) {
						?>
						<tr>
	...
	<?php
	if ( !empty( $clients_data['RunCloud'] ) ) {
		foreach ( $clients_data['RunCloud'] as $key => $client ) {
			?>
			<div id="client-details-modal-<?php echo $client['host_name']; ?>" uk-modal>
				<div class="server-modal uk-modal-dialog uk-modal-body">
					...
					<hr>
					<div>
						<a class="uk-button uk-button-danger uk-align-left uk-margin-remove-bottom" href="#delete-runcloud-client-<?php echo $client['host_name']; ?>" uk-toggle><?php esc_attr_e( 'DELETE', 'wp-cloud-server' ) ?></a>
					</div>
				</div>
				<div id="delete-runcloud-client-<?php echo $client['host_name']; ?>" uk-modal>
					<div class="server-modal uk-modal-dialog uk-modal-body">
						...
						<div class="uk-modal-footer uk-text-right">
							<form method="post" action="<?php echo admin_url( 'admin-post.php' ) ?>">
								<input type="hidden" name="action" value="handle_runcloud_client_action">
								<input type="hidden" name="wpcs_runcloud_client_action" value="delete">
								<input type="hidden" name="wpcs_runcloud_client_host_name" value="<?php echo $client['host_name'];?>">
								<input type="hidden" name="wpcs_runcloud_client_user_id" value="<?php echo $client['user_id'];?>">
								<?php wp_nonce_field( "handle_runcloud_client_action_nonce_{$client['host_name']}", "wpcs_handle_runcloud_client_action_nonce_{$client['host_name']}" ); ?>
...
add_action( 'wpcs_control_panel_tab_content', 'wpcs_runcloud_client_details_template', 10, 3 );