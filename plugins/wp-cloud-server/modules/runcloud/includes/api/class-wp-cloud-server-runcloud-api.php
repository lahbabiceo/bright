...
/**
 * WP Cloud Server - RunCloud Module API Class
 *
...
class WP_Cloud_Server_RunCloud_API {
	 ...
	 *  @var string
	 */
	private static $api_base_url = 'https://manage.runcloud.io/api/v2/';
	...
	public function __construct() {
		...
		self::$api_key		= get_option( 'wpcs_runcloud_api_key' );
		self::$api_secret 	= get_option( 'wpcs_runcloud_api_secret' );
	 ...
	 */
	public function wpcs_runcloud_check_api_health() {

		// Check if the health is temporarily cached.
		$health = get_transient( 'wpcs_runcloud_api_health' );
		...
		$api_response = wpcs_runcloud_call_api_health_check();
			
		update_option( 'wpcs_runcloud_api_last_health_response', $api_response );

		// if ( ! $api_response || $api_response['response']['http_code'] !== 200 ) {
		if ( wpcs_runcloud_api_response_valid( $api_response ) ) {

			// Can't connect. Not healthy.
			delete_transient( 'wpcs_runcloud_api_health' );
			update_option('wpcs_dismissed_runcloud_api_notice', FALSE );

			// Update the log event once for current failure
			if ( ! get_option( 'wpcs_runcloud_api_health_check_failed', false ) ) {
				wpcs_log_event( 'RunCloud', 'Failed', 'The RunCloud API Health Check Failed!' );
				update_option( 'wpcs_runcloud_api_health_check_failed', true );
			}
			return false;
		}
			
		// Update RunCloud Module Data
		WP_Cloud_Server_RunCloud_Settings::wpcs_runcloud_update_servers();

		// API connection is healthy. Cache result for three minutes
		wpcs_runcloud_set_transient( 'wpcs_runcloud_api_health', 'ok', 60 );
		
		if ( get_option( 'wpcs_runcloud_api_health_check_failed' ) ) {
			wpcs_runcloud_log_event( 'RunCloud', 'Success', 'The RunCloud API Health Check Completed Successfully' );

			// Clear the latched failed indication now that API is ok
			update_option( 'wpcs_runcloud_api_health_check_failed', false );
		}
	...
	public function call_api( $model, $api_data = null, $cache = false, $cache_lifetime = 900, $request = 'GET', $enable_response = false, $function = null ) {
		...
		if ( ( !isset( $module_list[ 'RunCloud' ]['status'] ) ) || ( 'active' !== $module_list[ 'RunCloud' ]['status'] ) ) {
			return false;
		}
		...
		$str = self::$api_key . ':' . self::$api_secret;
		...
		if ( ( $response_code < 200 ) || $response_code > 300 ) {
			...
			update_option( 'wpcs_runcloud_api_last_response', $response );
			//wpcs_RunCloud_api_response_data( $model, $body, $data, $api_data, $function );
			return false;
		}
	...
	private function get_do_api_response_cache( $model ) {
		return get_transient( 'wpcs_runcloud_api_response_' . md5( $model ) );
	}
	...
	private function set_do_api_response_cache( $model = null, $data = null, $lifetime = 900 ) {
		...
		wpcs_runcloud_set_transient( 'wpcs_runcloud_api_response_' . md5( $model ), $data, $lifetime );
	}
}