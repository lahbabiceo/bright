<?php
/**
 * The ServerPilot API functionality of the Plugin.
 *
...
class WP_Cloud_Server_ServerPilot_API {

	/**
	 *  ServerPilot API Official Base URL
	 *
	 *  @var string
	 */
	private static $api_base_url = 'https://api.serverpilot.io/v1';
	...
	public function __construct() {
		...
		self::$account_id	= get_option( 'wpcs_sp_api_account_id' );
		self::$api_key		= get_option( 'wpcs_sp_api_key' );
	 ...
	 */
	public function wpcs_serverpilot_check_api_health() {

		// Check if the health is temporarily cached.
		$health = get_transient( 'wpcs_sp_api_health' );
		...
		/**
		 *  Make API call to check if the user entered API settings are valid
		 */
		$data = array( 'user' => self::$account_id, 'key' => self::$api_key );
		$api_response = self::call_api( 'apps', $data, false, 900, 'GET', true, 'api_health' );
			
		update_option( 'wpcs_sp_api_last_health_response', $api_response );

		if ( ! $api_response || $api_response['response']['code'] !== 200 ) {

			// Can't connect. Not healthy.
			delete_transient( 'wpcs_sp_api_health' );
			update_option('wpcs_dismissed_sp_api_notice', FALSE );

			// Update the log event once for current failure
			if ( ! get_option( 'wpcs_sp_api_health_check_failed', false ) ) {
				wpcs_serverpilot_log_event( 'ServerPilot', 'Failed', 'The ServerPilot API Health Check Failed!' );
				update_option( 'wpcs_sp_api_health_check_failed', true );
			}
			return false;
		}
			
		// Update ServerPilot Module Data
		WP_Cloud_Server_ServerPilot_Settings::wpcs_sp_update_servers();
		
		// API connection is healthy. Cache result for fifteen minutes
		wpcs_sp_set_transient( 'wpcs_sp_api_health', 'ok', 900 );
		
		if ( get_option( 'wpcs_sp_api_health_check_failed' ) ) {
			
			wpcs_serverpilot_log_event( 'ServerPilot', 'Success', 'The ServerPilot API Health Check Completed Successfully' );

			// Clear the latched failed indication now that API is ok
			update_option( 'wpcs_sp_api_health_check_failed', false );
		}
		return true;
	 ...
	 */
	public function wpcs_serverpilot_check_api_setting() {
		
		$api_token = get_option( 'wpcs_sp_api_account_id' );
	 ...
	 */
	public function call_api( $endpoint, $api_data = null, $cache = false, $cache_lifetime = 900, $request = 'GET', $enable_response = false, $function = null, $id = null ) {

		$module_list = get_option( 'wpcs_module_list' );

		if ( ( !isset( $module_list[ 'ServerPilot' ]['status'] ) ) || ( 'active' !== $module_list[ 'ServerPilot' ]['status'] ) ) {
			return false;
		}
		...
		if ( $cache ) {
			$data_from_cache = self::wpcs_sp_get_api_response_cache( $endpoint );
		...
		if ( isset( $id ) && ( 'activate_ssl' == $function ) ) {
			$id = $id . '/' . 'ssl';	
		}

		// Include Server ID in GET request
		$endpoint = isset( $id ) ? $endpoint . '/' . $id : $endpoint;

		// Arguments for API call.
		$args = array(
			'headers'	=> array(
				'Content-Type'	=> 'application/json',
				'Authorization' => 'Basic ' . base64_encode( self::$account_id . ':' . self::$api_key),
			),
		...
		if ( ( $response_code < 200 ) || $response_code > 300 ) {
			...
			update_option( 'wpcs_serverpilot_api_last_response', $response );
			wpcs_sp_api_response_data( $endpoint, $body, $data, $api_data, $function );
			return false;
		}
		...
		if ( $cache ) {
			self::set_api_response_cache( $endpoint, $data, $cache_lifetime );
		}
		
		update_option( 'wpcs_serverpilot_api_last_response', $response );
		update_option( 'wpcs_serverpilot_api_last_data', $data );
	...
	/**
	 *  Get response for api endpoint from cache
	 *
	 *  @since  1.0.0
	 *  @param  string  $endpoint 	For what endpoint we want response
	 *  @return mixed   String 		if there's cached response, false if not
	 */
	private function wpcs_sp_get_api_response_cache( $endpoint ) {

		return get_transient( 'wpcs_sp_api_response_' . md5( $endpoint ) );
	 ...
	 */
	private function wpcs_sp_set_api_response_cache( $endpoint = null, $data = null, $lifetime = 900 ) {

		if ( ! $endpoint || ! $data ) {
			return;
		}

		set_transient( 'wpcs_sp_api_response_' . md5( $endpoint ), $data, $lifetime );
	}
