...
class WP_MAIL_LOG {
	...
	*/
	public function __construct() {
		...
		if ( is_admin() ) {
			require_once WML_PATH . 'classes/admin-notice.php';

			$admin_notice = new Admin_Notice();

			$wpv_notice = [];

			$wpv_notice = apply_filters( 'wpv-mail/admin_notices', $wpv_notice );
	...
	*/
	...
	public function wml_admin_enqueue_scripts() {
		...
		// wp_enqueue_script( 'wpml-setting-js', WML_URL . 'assets/js/settings.js', [], WML_VERSION, true );
		// wp_enqueue_style( 'wml-setting-style', WML_URL . 'assets/js/settings.css', [], WML_VERSION );
		wp_localize_script(
			'log-js',
			'WmlGlobalVar',
			[
	...
	public function wpv_mail_review() {
		...
		if ( isset( $_GET['wml_remind_later'] ) ) {
			...
			add_action( 'admin_notices', [ $this, 'wml_review_box' ] );
		}
	}
	/**
					...
					echo esc_html(
						add_query_arg(
							[
								...
								'wml_nonce'        => wp_create_nonce( 'wp_rest' ),
							]
						)
					)
					?>
	...
	public function wml_delete_entry() {
		...
		$query = "DELETE FROM {$table_name} WHERE DATE_FORMAT(captured_gmt,GET_FORMAT(DATE,'JIS')) <= '" . $date . "'";
